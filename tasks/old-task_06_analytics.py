...
def run_task_06(df: DataFrame, output_dir: str) -> None:
    ...
    df_rr = df.groupBy("country_name", "period").agg(
        (F.count_distinct(F.when(F.col("reported_indicators_count") > 0, F.col("orgUnit"))) / 
         F.count_distinct("orgUnit") * 100).alias("reporting_rate")
    )
